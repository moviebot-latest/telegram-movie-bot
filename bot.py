...
# ─── 409 FIX: Webhook delete + pending updates clear ───────
def clear_conflict():
    try:
    ...
    with Live(refresh_per_second=2, console=console) as live:
        while True:
            table = Table(box=box.ROUNDED, border_style="bright_black",
                          header_style="bold cyan", title="[bold red]📊 BOT STATS[/]", expand=False)
...
def startup_animation():
    steps = [
        ...
        "🧹 Clearing old conflicts...",
        "📡 Starting polling engine...",
        "✅ Bot is LIVE!",
    ]
    with Progress(SpinnerColumn(spinner_name="dots", style="bold red"),
...
def log(user, cmd, uid):
    ...
    console.print(f"[dim]{now}[/]  [bold red]{cmd:<12}[/]  [cyan]{user:<20}[/]  [dim white]id={uid}[/]")

# ─── HANDLERS ──────────────────────────────────────────────
@client.message_handler(commands=['start'])
def handle_start(msg):
    stats["total"] += 1; stats["start"] += 1
...
if __name__ == "__main__":
    ...
    try:
        client.infinity_polling(timeout=20, long_polling_timeout=10, skip_pending=True, none_stop=True)
